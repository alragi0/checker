...
if(strpos($message, "/http")===0 or strpos($message, "!http")===0 or strpos($message, ".http")===0){
 sendaction($chatId, typing);
            ...
            $git3 = file_get_contents("https://www.proxyscan.io/download?type=https");
                        $add_user = file_get_contents('temp/http@DiYxAr.txt');
            $add_user .= $git . "\n";
            file_put_contents("data/$chat_id/diyar.txt");
            file_put_contents('temp/http@DiYxAr.txt', $add_user);
                        $add_user = file_get_contents('temp/http@DiYxAr.txt');
            $add_user .= $git1 . "\n";
            file_put_contents("data/$chat_id/diyar.txt");
            file_put_contents('temp/http@DiYxAr.txt', $add_user);
      ...
      $post = array(
        ...
        'document' => new CURLFile(realpath('temp/http@DiYxAr.txt'))
        );
...
if(strpos($message, "/socks4")===0 or strpos($message, "!socks4")===0 or strpos($message, ".socks4")===0){
 sendaction($chatId, typing);
            file_put_contents("temp/socks4@DiYxAr.txt", file_get_contents("https://api.proxyscrape.com/?request=getproxies&proxytype=socks4&timeout=5000&country=all"));
            $last_updated = file_get_contents('https://api.proxyscrape.com?request=lastupdated&proxytype=socks4');
                       $git = file_get_contents("https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/socks4.txt");
                        $add_user = file_get_contents('temp/socks4@DiYxAr.txt');
            $add_user .= $git . "\n";
            file_put_contents("data/$chat_id/diyar.txt");
            file_put_contents('temp/socks4@DiYxAr.txt', $add_user);
                       $git1 = file_get_contents("https://www.proxyscan.io/download?type=socks4");
                        $add_user = file_get_contents('temp/socks4@DiYxAr.txt');
            $add_user .= $git1 . "\n";
            file_put_contents("data/$chat_id/diyar.txt");
            file_put_contents('temp/socks4@DiYxAr.txt', $add_user);
                        $file = "temp/socks4@DiYxAr.txt";
            $no_of_lines = count(file($file));
            $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, 'https://api.telegram.org/bot'.$botToken.'/sendDocument');
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
      ...
      $post = array(
        ...
        'document' => new CURLFile(realpath('temp/socks4@DiYxAr.txt'))
        );
      curl_setopt($ch, CURLOPT_POSTFIELDS, $post);
...
if(strpos($message, "/socks5")===0 or strpos($message, "!socks5")===0 or strpos($message, ".socks5")===0){
    sendaction($chatId, typing);
    file_put_contents("temp/socks5@DiYxAr.txt", file_get_contents("https://api.proxyscrape.com/?request=getproxies&proxytype=socks5&timeout=5000&country=all"));
    ...
    $add_user = file_get_contents('temp/socks5@DiYxAr.txt');
    $add_user .= $git . "\n";
    file_put_contents("data/$chat_id/diyar.txt");
    file_put_contents('temp/socks5@DiYxAr.txt', $add_user);
    $git1 = file_get_contents("https://www.proxyscan.io/download?type=socks5");
    $add_user = file_get_contents('temp/socks5@DiYxAr.txt');
    $add_user .= $git1 . "\n";
    file_put_contents("data/$chat_id/diyar.txt");
    file_put_contents('temp/socks5@DiYxAr.txt', $add_user);
    $file = "temp/socks5@DiYxAr.txt";
    $no_of_lines = count(file($file));
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://api.telegram.org/bot'.$botToken.'/sendDocument');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    "reply_to_message_id"=> $message_id,
    'reply_markup' => $keyboard,
    'document' => new CURLFile(realpath('temp/socks5@DiYxAr.txt'))
    );
    curl_setopt($ch, CURLOPT_POSTFIELDS, $post);
