...
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot6191669055:AAGj0hKwQnRNjCfLbMY9TQD8GGYjpWyFTI8/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
Note-⟩ If you face any kind of bug in this bot, please report it to the developer at @YYNXX </b>");
$free = json_encode($keyboard);
        file_get_contents("https://api.telegram.org/bot6191669055:AAGj0hKwQnRNjCfLbMY9TQD8GGYjpWyFTI8/editMessageText?chat_id=$cchatid2&text=$freecommands&message_id=$cmessage_id2&parse_mode=HTML&reply_markup=$free");
...
if ($cdata2 == "buy"){
    ...
    $keyboard = [
    'inline_keyboard' => [
         [['text' => 'Buy Now', 'url' => 'https://t.me/YYNXX'], 
         ['text' => 'Premium', 'callback_data' => 'paid'],
         ['text' => 'Finalize', 'callback_data' => 'end']]
        ]];
...
file_get_contents("https://api.telegram.org/bot6191669055:AAGj0hKwQnRNjCfLbMY9TQD8GGYjpWyFTI8/editMessageText?chat_id=$cchatid2&text=$finalize&message_id=$cmessage_id2&parse_mode=HTML");
}
...
THIS BOT IS MADE WITH ♥️ BY  @YYNXX </b>",
 'parse_mode'=>'HTML',
 'reply_markup' => $free,
 ]);
        exit();
...
$premium_id = (array("926877758","6166758983"));
$premiumgp = (array("-1001978439944"));
        $keyboard = json_encode([
'inline_keyboard' => [
[['text' => "OWNER", 'url' => "https://t.me/YYNXX"],]
]]);
                $keyboard1 = json_encode([
'inline_keyboard' => [
[['text' => "CHANNEL", 'url' => "https://t.me/ALRAGI1"],
['text' => "GROUP", 'url' => "https://t.me/YYNXX7"],]
]]);

// if(!in_array($chatId, $premiumgp)){
	// $tch = json_decode(file_get_contents("https://api.telegram.org/bot6209431758:AAHM6M2TtdWx9txXE4bCQ7Fkd71f80TJUcE/getChatMember?chat_id=@diyxar&user_id=".$gId))->result->status;
...
if(strpos($message, '!sk') === 0 or strpos($message, '/sk') === 0 or strpos($message, '.sk') === 0){
    $keyboard = [
    'inline_keyboard' => [[['text' => 'Features', 'callback_data' => 'paid'], ['text' => 'Buy', 'callback_data' => 'buy'], ['text' => 'Buy Now', 'url' => 'https://t.me/YYNXX'],]]];
...
edit_message($chatId,$message_id_1,$keyboard, "<b>═════ 『 𝗔𝗹𝗿𝗮𝗴𝗶 † 』════ %0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>Bot: @CheckerYemenbot </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
edit_message($chatId,$message_id_1,$keyboard, "<b>═════ 『 𝗔𝗹𝗿𝗮𝗴𝗶 † 』════%0A❌ DEAD KEY ❌</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>Bot: @CheckerYemenbot </b>");
}else{
edit_message($chatId,$message_id_1,$keyboard, "<b>═════ 『 𝗔𝗹𝗿𝗮𝗴𝗶 † 』════%0A✅LIVE KEY✅</b>%0A<u>KEY:</u> <code>${fii}xxxxxxxxxxxxxxxxx${newstring}</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>Bot: @CheckerYemenbot</b>");
// sleep(10);
...
function bot($method,$datas=[]){
    $url = "https://api.telegram.org/bot6191669055:AAGj0hKwQnRNjCfLbMY9TQD8GGYjpWyFTI8/".$method;
    $ch = curl_init();
...
function sendMessage1($message) {
        ...
        $url = $GLOBALS[website]."/sendMessage?chat_id=-1001978439944&text=".$message."&parse_mode=HTML";
        file_get_contents($url);
...
global $botToken;
$url = "https://api.telegram.org/bot".$botToken."/editMessageText?chat_id=".$chatId."&message_id=".$message_id."&text=••MASS CHECKER%0A".$message."%0A••• BOT BY: @xcheckerccbot&parse_mode=HTML";
...
$final = json_encode($json_array);
    $result = "<i>SORRY TO SAY THAT</i>\n <b> YOU HAVE BEEN u760616334_alragiTED TO FREE USER BECAUSE YOU DONT HAVE CREDITS NOW \n YOU CAN BUY CREDITS NOW BY HITTING /buy </b>";
...
<i>take group access from @YYNXX to use here</i></b>");
